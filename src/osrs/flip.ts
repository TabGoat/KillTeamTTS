...
// ── GE mechanics note ─────────────────────────────────────────────────────
//
//  API `low`  = insta-buy price  → what YOU PAY to buy instantly
//  API `high` = insta-sell price → what YOU RECEIVE when selling instantly
//
//  GE tax (1%, capped at 5M gp) is paid by the SELLER on the sell price.
//  Items under 100 gp are tax-exempt.
//
//  Spread = high - low   (gross profit per item before tax)
//  Profit = spread - tax(high)
//
// ── Types ─────────────────────────────────────────────────────────────────
...
export interface FlipOpportunity {
  ...
  members: boolean;
  examine: string;
  buyPrice: number;           // what you pay  (API: low  = insta-buy price)
  sellPrice: number;          // what you get  (API: high = insta-sell price)
  spread: number;             // gross margin before tax  (high - low)
  tax: number;                // GE tax on sell price (2%, max 5M gp, exempt < 50gp)
  profit: number;             // net profit per item  (spread - tax)
  roi: number;                // profit / buyPrice × 100
  buyLimit: number;           // GE buy limit per 4h window
  maxProfit4h: number;        // profit × min(buyLimit, volume4h) — realistic ceiling
  hourlyVolume: number;       // items traded per hour (from 1h endpoint)
  trend: 'up' | 'down' | 'stable' | 'unknown';
  trendPct: number;           // % price change 5m vs 1h avg
  dataAge: number;            // seconds since the older of highTime / lowTime
  // ── Action plan (time-horizon-aware) ──────────────────────────────────────
  recommendedBuyPrice: number;   // offer this to be first in buy queue (low + 1)
  recommendedSellPrice: number;  // offer this to undercut sell queue (high - 1)
  recommendedQty: number;        // how many to buy given the time horizon
  totalInvestment: number;       // recommendedBuyPrice × recommendedQty
  expectedProfit: number;        // net profit for the full recommended quantity
  estimatedFillMinutes: number;  // how long to fill the buy order
  marketImpact: 'low' | 'medium' | 'high'; // whether this qty moves the market
  buyFillMinutes: number;    // guaranteed ≤ timeHours × 60 — strict buy window
}
...
export interface FlipFilters {
  ...
  f2pOnly?: boolean;
  limit?: number;
  sortBy?: 'profit4h' | 'roi' | 'volume' | 'profit' | 'score' | 'profit_total';
  timeHours?: number;
  totalBudget?: number;  // total gp to invest across ALL slots — split equally per item
}

interface LatestEntry { high: number | null; highTime: number | null; low: number | null; lowTime: number | null }
interface AvgEntry    { avgHighPrice: number | null; avgLowPrice: number | null; highPriceVolume: number; lowPriceVolume: number }
interface TimePoint   { timestamp: number; avgHighPrice: number | null; avgLowPrice: number | null }
...
const GE_TAX_CAP      = 5_000_000; // GE tax is capped at 5M gp per transaction

// ── API helpers ───────────────────────────────────────────────────────────

async function fetchJson<T>(path: string): Promise<T> {
  const res = await fetch(`${PRICES_API}${path}`, { headers: { 'User-Agent': USER_AGENT } });
  if (!res.ok) throw new Error(`Prices API ${path}: ${res.status}`);
 ...
 */
function calcTax(sellPrice: number): number {
...
function classifyTrend(avg5m: AvgEntry | undefined, avg1h: AvgEntry | undefined): { trend: FlipOpportunity['trend']; trendPct: number } {
  const price5m = avg5m?.avgHighPrice ?? avg5m?.avgLowPrice;
  const price1h = avg1h?.avgHighPrice ?? avg1h?.avgLowPrice;
  if (!price5m || !price1h || price1h === 0) return { trend: 'unknown', trendPct: 0 };
...
function computeActionPlan(
  ...
  hourlyVolume: number,
  timeHours: number,
): Pick<FlipOpportunity, 'recommendedBuyPrice' | 'recommendedSellPrice' | 'recommendedQty' | 'totalInvestment' | 'expectedProfit' | 'estimatedFillMinutes' | 'marketImpact' | 'buyFillMinutes'> {
  ...
  const marketImpact: FlipOpportunity['marketImpact'] =
    impactRatio >= 0.8 ? 'high' : impactRatio >= 0.4 ? 'medium' : 'low';
...
export async function findFlipOpportunities(filters: FlipFilters = {}): Promise<FlipOpportunity[]> {
  const {
    minProfit   = 200,
    minRoi      = 0.5,
    minVolume   = 10,
  ...
  const [mapping, latestData, data5m, data1h, data24h] = await Promise.all([
    getMapping(),
    fetchJson<{ data: Record<string, LatestEntry> }>('/latest'),
    fetchJson<{ data: Record<string, AvgEntry> }>('/5m'),
    fetchJson<{ data: Record<string, AvgEntry> }>('/1h'),
    fetchJson<{ data: Record<string, AvgEntry> }>('/24h'),
  ]);

  const mapById = new Map<number, PriceMapping>(mapping.map((m) => [m.id, m]));
  ...
  for (const [idStr, latest] of Object.entries(latestData.data)) {
    ...
    if (maxBuyPrice && buyPrice > maxBuyPrice) continue;
    if (membersOnly && !item.members)          continue;
  ...
  const score = (o: FlipOpportunity): number =>
    o.roi * Math.log10(Math.max(o.hourlyVolume, 1)) * Math.log10(Math.max(o.profit, 1));
  ...
  if (totalBudget && totalBudget > 0 && sliced.length > 0) {
    ...
    const budgeted: FlipOpportunity[] = [];
    for (const o of sliced) {
      const budgetQty = Math.floor(perItemBudget / o.recommendedBuyPrice);
      if (budgetQty < 1) continue;  // can't afford even one — skip slot
...
export interface ItemFlipDetail extends FlipOpportunity {
...
export async function analyseItemFlip(item: PriceMapping, timeHours = 4): Promise<ItemFlipDetail> {
  const nowSec = Math.floor(Date.now() / 1000);

  const [latestData, avg5m, avg1h, avg24h, timeseries] = await Promise.all([
    fetchJson<{ data: Record<string, LatestEntry> }>(`/latest?id=${item.id}`),
    fetchJson<{ data: Record<string, AvgEntry> }>(`/5m?id=${item.id}`),
    fetchJson<{ data: Record<string, AvgEntry> }>(`/1h?id=${item.id}`),
    fetchJson<{ data: Record<string, AvgEntry> }>(`/24h?id=${item.id}`),
    fetchJson<{ data: TimePoint[] }>(`/timeseries?timestep=5m&id=${item.id}`),
  ]);
  ...
  if (stale)   recommendation = '⚠️ Price data is stale (>30 min old) — do not trade on this.';
  else if (anomaly) recommendation = '⚠️ Spread is abnormally wide — possible manipulation or illiquid market.';
  else if (trend === 'up'   && roi > 2) recommendation = '📈 Strong buy — price trending up + healthy margin.';
  else if (trend === 'up'   && roi < 1) recommendation = '📈 Trending up but margin is thin — watch for slippage.';
  else if (trend === 'down' && roi > 3) recommendation = '⚠️ High margin but price falling — flip fast or skip.';
  else if (trend === 'down' && roi < 1) recommendation = '❌ Avoid — price falling and margin is thin.';
  else recommendation = '🔍 Neutral — standard flip opportunity.';

  if (!stale && !anomaly && volatility > 5)     recommendation += ' High volatility — act quickly.';
  if (!stale && !anomaly && hourlyVolume < 20)  recommendation += ' Low volume — orders may take time to fill.';
...
function calcChangePct(prices: number[], lookback: number): number {
