...
# Optional imports for secret backends
try:
    import boto3
except Exception:
    boto3 = None
...
logger = logging.getLogger("sunor_agent_secure")
...
def require_auth(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not token:
            return jsonify({"error": f"Server not configured: missing {AGENT_TOKEN_ENV}"}), 403
        header = request.headers.get("X-AGENT-AUTH")
...
def get_sunor_key():
    # Try env var first
    key = os.environ.get(SUNOR_KEY_ENV)
    if key:
        return key

    # Try AWS Secrets Manager
    aws_name = os.environ.get("AWS_SECRET_NAME")
    if aws_name and boto3:
        try:
            client = boto3.client('secretsmanager')
            resp = client.get_secret_value(SecretId=aws_name)
            secret_string = resp.get('SecretString')
            if secret_string:
                try:
                    data = json.loads(secret_string)
                    return data.get('SUNOR_API_KEY') or data.get('api_key') or data.get('key') or secret_string
                except Exception:
                    return secret_string
        except Exception:
            logger.exception("Failed to read secret from AWS Secrets Manager")

    # Try Azure Key Vault
    vault_name = os.environ.get('AZURE_VAULT_NAME')
    secret_name = os.environ.get('AZURE_SECRET_NAME')
    if vault_name and secret_name and DefaultAzureCredential and SecretClient:
        try:
            ...
            try:
                data = json.loads(val)
                return data.get('SUNOR_API_KEY') or val
            except Exception:
                return val
        except Exception:
            logger.exception("Failed to read secret from Azure Key Vault")
...
def worker_job(task_id):
    ...
    key = get_sunor_key()
    if not key:
        db_update(task_id, status="failure", error="SUNO API key not configured")
        return
...
if __name__ == "__main__":
    ...
    logger.info("Starting secure Sunor agent on port %s", port)
